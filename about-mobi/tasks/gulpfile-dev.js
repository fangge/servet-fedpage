module.exports = function(gulp, plugins) {
    ...
    gulp.task('dev_slice2css', function(){
        var fs = require('fs')
        var path = require('path')
        var async = require('uglify-js/node_modules/async')
        var getPixels = require('get-pixels')
        var ejs = require('ejs')

        var classnameRule = function(fileName, p){
            var relPath = path.relative('src/img', path.dirname(p))
        ...
        async.series([
            ...
            function(next){
                ...
                files = glob.sync("src/img/**", {nodir:true})
                ...
                function iterator(f, _next){
                    getPixels(f, function (err, pixels) {
                        ...
                        arr.push({
                            ...
                            width: 'pxrem('+pixels.shape[0]+'px)',
                            height: 'pxrem('+pixels.shape[1]+'px)'
                        })
                        _next(null)
                    })
                }
